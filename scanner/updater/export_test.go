	...
	tests := []struct {
		...
		},
		{
			...
			status: ExportStatus{
				Updaters: []UpdaterStatus{
					{Name: "alpine", Status: "success"},
					{Name: "nvd", Status: "success"},
				},
			},
			expected: false,
		},
		{
			...
			status: ExportStatus{
				Updaters: []UpdaterStatus{
					{Name: "alpine", Status: "success"},
					{Name: "photon", Status: "failed", Error: "404 not found"},
				},
			},
			expected: true,
		},
		{
			...
			status: ExportStatus{
				Updaters: []UpdaterStatus{
					{Name: "alpine", Status: "failed", Error: "timeout"},
					{Name: "photon", Status: "failed", Error: "404 not found"},
				},
			},
			expected: true,
...
func TestExportStatus_SuccessCount(t *testing.T) {
	status := ExportStatus{
		Updaters: []UpdaterStatus{
			{Name: "alpine", Status: "success"},
			{Name: "nvd", Status: "success"},
			{Name: "photon", Status: "failed", Error: "404"},
			{Name: "debian", Status: "success"},
		},
	}
...
func TestExportStatus_FailureCount(t *testing.T) {
	status := ExportStatus{
		Updaters: []UpdaterStatus{
			...
			{Name: "nvd", Status: "success"},
			{Name: "photon", Status: "failed", Error: "404"},
			{Name: "oracle", Status: "failed", Error: "timeout"},
		},
	}
...
func TestWriteStatusFile(t *testing.T) {
	...
	status := &ExportStatus{
		Updaters: []UpdaterStatus{
			{Name: "alpine", Status: "success", LastAttempt: now},
			{Name: "photon", Status: "failed", Error: "404 not found", LastAttempt: now},
		},
	}
	...
	assert.Equal(t, "success", alpineStatus.Status)
	assert.Empty(t, alpineStatus.Error)

	require.NotNil(t, photonStatus)
	assert.Equal(t, "failed", photonStatus.Status)
